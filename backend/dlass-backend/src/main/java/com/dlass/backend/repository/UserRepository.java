...
public interface UserRepository extends MongoRepository<User, String> {
    ...
    @Query("{ 'email': ?0, $or: [ { 'isActive': true }, { 'active': true }, { 'isActive': { $exists: false }, 'active': { $exists: false } } ] }")
    Optional<User> findByEmailAndIsActiveTrue(String email);

    /** Active users by role (for admin user list) */
    @Query("{ 'role': ?0, $or: [ { 'isActive': true }, { 'active': true }, { 'isActive': { $exists: false }, 'active': { $exists: false } } ] }")
